...
from utils import CTDataset, DegradationSimulatorCT
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--output-dir", type=str, default="weights/ct")
    ...
    parser.add_argument("--delta-max", type=float, default=1.1)
...
def main() -> None:
    ...
    logger = build_logger(workspace, args.log_dir, "train_ct")
    device = torch.device(args.device)
    dataset = CTDataset(args.train_file, args.image_size)
    ...
    simulator = DegradationSimulatorCT(device=device)
    ...
    for epoch in range(args.epochs):
        ...
        progress = tqdm(dataloader, dynamic_ncols=True, desc=f"train_ct epoch {epoch}")
        ...
        checkpoint_path = output_dir / f"ct_epoch_{epoch + 1:03d}.pt"
