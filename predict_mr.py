...
from utils.data import build_output_path, load_mr_nifti, read_paired_paths, save_mr_nifti
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--results-dir", type=str, default="outputs/mr/testA")
...
def main() -> None:
    ...
    logger = build_logger(workspace, args.log_dir, "predict_mr")
    ...
    for start in tqdm(range(0, len(pairs), args.batch_size), desc="predict_mr", dynamic_ncols=True):
        batch_pairs = pairs[start:start + args.batch_size]
        batch_input_paths = [pair[0] for pair in batch_pairs]
        batch_images = [load_mr_nifti(path, args.image_size) for path in batch_input_paths]
        ...
        for index, input_path in enumerate(batch_input_paths):
            ...
            save_mr_nifti(batch_output[index:index + 1], input_path, output_path)
