...
from utils.data import build_output_path, load_ct_dicom, read_paired_paths


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser()
    parser.add_argument("--test-file", type=str, required=True)
    parser.add_argument("--predictions-dir", type=str, default="outputs/ct/testA")
...
def main() -> None:
    ...
    logger = build_logger(workspace, args.log_dir, "evaluate_ct")
    ...
    csv_path = workspace / args.log_dir / f"evaluate_ct_{datetime.now():%Y%m%d_%H%M%S}.csv"
    ...
    with open(csv_path, "w", newline="", encoding="utf-8") as handle:
        ...
        for input_path, target_path in tqdm(pairs, desc="evaluate_ct", dynamic_ncols=True):
            ...
            if not prediction_path.exists():
                ...
                continue
            prediction = torch.from_numpy(load_ct_dicom(prediction_path, args.image_size)).unsqueeze(0).to(device)
            target = torch.from_numpy(load_ct_dicom(target_path, args.image_size)).unsqueeze(0).to(device)
            psnr, ssim, rmse, lpips = metric.compute_all(prediction, target, mode="CT")
